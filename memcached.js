...
{
    this.client.socket.send(this.name + " " + this.key + " " + this.value + "\r\n");
}
...
setCmd.prototype.process = function()
{
    ...
    if (this.name == cas)
        cas += " " + this.cas;
    if (this.name != "cas")
        this.client.socket.send(this.name + " " + this.key +  " " + this.flag + " " + this.exptime + " " + this.value.length + cas + "\r\n" + this.value + "\r\n");
}
...
{
    this.client.socket.send("delete " + this.key +  " " + this.timeout + " " + "\r\n");
}
...
getCmd.prototype.process = function()
{
    ...
    if (!this.keys.join)
    {
        ...
        keys = this.keys.join(" ");
    if (this.getCas)
        this.client.socket.send("gets " + keys + "\r\n");
    else
        this.client.socket.send("get " + keys + "\r\n");
}
